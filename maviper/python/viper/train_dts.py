...
def collect_dataset(env, n_agents, max_timesteps, collection_method, max_samples, maddpg=None, bins=None):
    ...
    for i in range(max_samples):
        ...
        for a in range(n_agents):
            # convert all datapoints to bins
            ...
            for entry in obs[a]:
                ...
                for b, bin in enumerate(bins):
                    ...
                    if entry < ub and entry >= lb:
                        ...
                        break 
                    lb = ub 
            assert len(binned_obs) == len(obs[a])
            dataset[a].append((binned_obs, actions[a], rewards[a], nobs[a]))
...
def train_dts(parameters):
    ...
    # logging 
    save_folder = CSV_RESULTS_FOLDER + scenario_name + '/max-depth' + str(max_depth) + '/'
    if not os.path.exists(save_folder):
        os.makedirs(save_folder)
    save_folder = setup_save_folder(save_folder)
    save_file_name = parameters.agent_type
    
    # TODO: not saving any metrics right now -- just debugging to see if the implementation works 
    ...
    # TODO: merge with more general way to get teams 
