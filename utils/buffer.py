...
class ReplayBuffer(object):
    ...
    def __init__(self, max_steps, num_agents, obs_dims, ac_dims):
        ...
        self.ep = [-1 for _ in range(max_steps)]
        self.epr = dict()

    def __len__(self):
        return self.filled_i

    def push(self, observations, actions, rewards, next_observations, dones, cur_ep):
        ...
        # for i in range(self.curr_i, self.curr_i + nentries):
        #     self.ep[i] = cur_ep
        #     self.epr[cur_ep] = self.epr.get(cur_ep, list()) + [i]
    ...
    def sample(self, N, to_gpu=False, norm_rews=True):
        # N = N // 25
        # all_ep = list(set(self.ep))
        # if -1 in all_ep:
        #     all_ep.remove(-1)
        # ep_inds = np.random.choice(np.arange(len(all_ep)), size=N,
        #                            replace=False)
        # inds = []
        # for i in ep_inds:
        #     inds += self.epr[all_ep[i]]
        inds = np.random.choice(np.arange(self.filled_i), size=N,
                                replace=False)
        if to_gpu:
            # cast = lambda x: Variable(torch.tensor(x, device='cuda', dtype=torch.float), requires_grad=False)
            cast = lambda x: torch.from_numpy(x).float().cuda()
        else:
